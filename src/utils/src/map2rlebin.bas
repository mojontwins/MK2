' map2rlebin v0.1
' fbc map2rlebin.bas cmdlineparser.bas mtparser.bas
...
Dim As String mandatory (3) = { "in", "out", "size", "rle" }
...
Print "map2rlebin v0.1.20191202 ~ ";
...
' RLE format
rleformat = Val (sclpGetValue ("rle"))
Select Case rleformat
	Case 44:
		MAXRUN = 16: ANDVALUE = 15: NUMBITS = 4

	Case 53:
		MAXRUN = 8: ANDVALUE = 31: NUMBITS = 5

	Case 62:
		MAXRUN = 4: ANDVALUE = 63: NUMBITS = 6
End Select
...
For nPant = 0 To maxPants - 1
	...
	For i = 1 To scrW*scrH-1
		...
		If BigMap (nPant, i) <> d Or n = MAXRUN Then
			cMap (nPant, cMapI) = (d And ANDVALUE) Or ((n - 1) Shl NUMBITS)
