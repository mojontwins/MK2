...
if (hitter_on && killable && hitter_hit == 0) {
		...
		if (
			hitter_frame <= 3 && 
			collide_pixel (hitter_x + (p_facing ? 6 : 1), hitter_y + 3, gpen_cx, gpen_cy)
		) 
		...
		if (
			...
			hitter_frame < 7 &&
			(
				(p_up == 0 && collide_pixel (hitter_x + (p_facing ? 6 : 1), hitter_y + 3, gpen_cx, gpen_cy)) ||
		...
		if (
			hitter_frame < 5 &&
			collide_pixel (hitter_x + (p_facing ? 14 : 1), hitter_y + 3, gpen_cx, gpen_cy)
		) 
	#endif
	{
		hitter_hit = 1;
